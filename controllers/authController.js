...
export const verifyController = async (req, res) => {
    ...
    try {
      ...
      console.error(error);
      res.status(500).json({ error: "Greška pri verifikovanju korisnika." });
    }
  };
...
export const searchUserController = async (req,res) =>{
 ...
 // Promenite putanju prema modelu ako je potrebno

 export const searchOrderController = async (req, res) => {
  try {
    const { keyword } = req.params; // Dobijanje ključne reči za pretragu iz params

    const user = await userModel.findOne({ name: keyword });

    if (user) {
      // Ako korisnik sa tim imenom postoji, možete pristupiti njihovom ID-ju
      const userId = user._id;
      
      // Sada možete koristiti userId u pretrazi po narudžbinama
      const results = await orderModel.find({
        $or: [
          { status: { $regex: keyword, $options: "i" } },
          { buyer: userId }, // Pretraga po ID-ju kupca
        ],
      })
      .populate({
        path: "products",
      ...
      // Ako korisnik sa tim imenom ne postoji, možete rukovati ovom situacijom ovde
      res.json([]);
    }
  } catch (error) {
    console.log(error);
    res.status(400).send({
      success: false,
      message: "Greška prilikom pretrage",
      error,
    });
  }
};
